...
use super::env::{TestSetupError, run_to_end, variable};
use crate::database::migrate;

/// The prefix of every test database's name.
pub const TEST_DATABASE_PREFIX: &str = "lpa_test_";
/// The role that owns the test databases, and that their pools act as: the admin server's.
pub const TEST_DATABASE_OWNER: &str = "life_pixel_admin";
/// The variable naming the server, as a role that may create databases: the server's tests'.
...
impl TestDatabase {
    /// Creates, comments and migrates `lpa_test_<16 random hexadecimal digits>`, after removing
    ...
    /// The URL of the database, acting as [`TEST_DATABASE_OWNER`], for a command run on it; the
    /// database is still dropped with the value.
    ///
    /// # Errors
    ///
    /// When `LP_TEST_DATABASE_URL` is missing.
    pub fn url(&self) -> Result<String, TestSetupError> {
...
/// Whether `name` is `lpa_test_` and 16 lowercase hexadecimal digits.
...
mod tests {
    ...
    fn only_test_databases_are_dropped() {
        assert!(is_test_database("lpa_test_0123456789abcdef"));
        for name in [
            "life_pixel_admin",
            "lpa_test_",
            "lp_test_0123456789abcdef",
            "lpa_test_0123456789ABCDEF",
            "lpa_test_x; drop",
        ] {
    ...
    fn a_database_url_names_the_database_and_the_owner_role() {
        ...
        assert_eq!(
            database_url(server, "lpa_test_0123456789abcdef"),
            "postgres://postgres:local@127.0.0.1:5460/lpa_test_0123456789abcdef\
             ?options=-c%20role%3Dlife_pixel_admin"
        );
