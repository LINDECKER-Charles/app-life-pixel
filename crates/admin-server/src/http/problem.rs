//! Problems, as the server's: every error answers `application/problem+json`, with a stable code
//! and its parameters, never a sentence. The codes the admin server answers with, and their
//! statuses, are in [`statuses`].
...
/// An error on its way to the console.
...
impl Problem {
    ...
    /// the console only learns `internal.error`.
    #[must_use]
    ...
    /// The problem, telling the console to wait `seconds` before trying again.
    #[must_use]
