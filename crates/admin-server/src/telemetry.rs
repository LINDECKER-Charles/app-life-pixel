//! Telemetry, as the server's: JSON logs on stdout filtered by `RUST_LOG`, OTLP traces when
//! `OTEL_EXPORTER_OTLP_ENDPOINT` is set, Prometheus metrics on the metrics listener. Never a
//! password, a TOTP code or secret, a session token or an email address in any of them.
...
pub const SERVICE_NAME: &str = "life-pixel-admin-server";
