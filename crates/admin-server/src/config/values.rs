//! The value types of the configuration: environment, origins, secrets, keys, log filter, as the
//! server's.
...
/// The environment the admin server administers: in its banner, its logs and its traces.
...
impl Environment {
    /// The name, as `LPA_ENVIRONMENT` spells it.
    #[must_use]
...
impl FromVariable for Vec<Origin> {
    const EXPECTED: &'static str = "origins separated by commas, such as http://localhost:4263";
...
impl FromVariable for LogFilter {
    const EXPECTED: &'static str =
        "tracing directives such as info or life_pixel_admin_server=debug";
