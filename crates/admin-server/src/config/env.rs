...
//! that names the variable without ever quoting its value — as the server's.
...
/// A variable the admin server cannot start without. The message names the variable and what it
...
impl<'a> Env<'a> {
    ...
    pub fn optional(&self, variable: &str) -> Option<String> {
...
impl FromVariable for SocketAddr {
    const EXPECTED: &'static str = "an address and a port, such as 127.0.0.1:8463";
