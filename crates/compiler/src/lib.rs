...
//! comes from `life-pixel-core`'s rendering, so an export shows what the editor shows. The crate
//! is pure: no I/O, no clock, no randomness, so the same animation always gives the same bytes.
